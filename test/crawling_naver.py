...
def ex_tag(sid, page):
    ...
    for a in a_tag:
        if "href" in a.attrs:  # href가 있는것만 고르는 것
            ...
            if (
                href.startswith("https://n.news.naver.com/mnews/article/")
                and "/comment/" not in href
            ):
                tag_lst.append(href)
...
def art_crawl(all_hrefs, sid, index):
    ...
    publisher_selector = (
        "#ct > div.media_end_head.go_trans > div.media_end_head_top._LAZY_LOADING_WRAP"
    ...
    publisher = (
        publisher_tag["alt"] if publisher_tag and "alt" in publisher_tag.attrs else ""
    )
