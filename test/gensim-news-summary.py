...
def clean_text(text):
    ...
    text = re.sub(r'\(.*?(사진|뉴스|제공).*?\)', '', text)  # 소괄호
    text = re.sub(r'\[.*?(사진|뉴스|제공).*?\]', '', text)  # 대괄호
    text = re.sub(r'\{.*?(사진|뉴스|제공).*?\}', '', text)  # 중괄호
...
df['summary'] = df['cleaned_main'].apply(lambda x: summarize(x, sent_num=2))

# summary 열을 '.' 기준으로 줄바꿈하여 저장
df['summary'] = df['summary'].apply(lambda x: x.replace('. ', '.\n'))

# 요약된 데이터를 저장할 엑셀 파일 경로
output_file_path = './data/article_df_240821_gensim_summary.xlsx'
